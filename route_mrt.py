...
if __name__ == '__main__':
    ...
    # path
    path_output_file = open('mrt_analysis/path_' + mrt_file_name.split('.')[0] + '_output.csv', 'w')
    csv_writer = csv.writer(path_output_file, delimiter=',')
    for i in range(len(new_key_list)):
        csv_writer.writerow([new_key_list[i], new_value_list[i]])

    out_output_file = open('mrt_analysis/out_' + mrt_file_name.split('.')[0] + '_output.csv', 'w')
    csv_writer = csv.writer(out_output_file, delimiter=',')
    for i in range(len(out_list)):
        csv_writer.writerow([out_list[i], out_v_list[i]])

    des_output_file = open('mrt_analysis/des_' + mrt_file_name.split('.')[0] + '_output.csv', 'w')
